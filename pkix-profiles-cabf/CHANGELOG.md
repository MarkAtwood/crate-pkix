...
- **Crate-private OID constants** for Subject DN attributes referenced by
  the new tier's `required_leaf_subject_dn_attrs` rule: `OID_SURNAME`
  (2.5.4.4), `OID_SERIAL_NUMBER` (2.5.4.5), `OID_GIVEN_NAME` (2.5.4.42),
  `OID_PSEUDONYM` (2.5.4.65). Crate-private constant for the tier-marker
  policy OID: `CABF_SMIME_INDIVIDUAL_VALIDATED_POLICY`
  (2.23.140.1.5.4.1). (PKIX-jbvb.3.)
- **Test fixtures** `smime-individual-validated-self-signed-365d.der`
  (givenName + surname + serialNumber form) and
  `smime-individual-pseudonym-self-signed-365d.der` (pseudonym +
  serialNumber form, exercises the `AnyOf` branch of the DN rule) in
  `pkix-path/tests/fixtures/policy-checks/`. Generated by new sibling
  script `gen-smime-tier-fixtures.py` modeled after zlint's
  `smime_leg1_iv_eff1.pem` but including the BR §7.6 Subject DN
  attributes zlint's published fixture omits. (PKIX-jbvb.3.)
