...
fn validity_max_cap_function_100d_phase() {
    ...
    // after 2027-03-15.
    //
    // Oracle: CA/B Forum Ballot SC-081 — the 100-day threshold is 2027-03-15T00:00:00Z
    // and the cap value is exactly 100 * 86_400 = 8_640_000 seconds.
...
// ---------------------------------------------------------------------------
// SC-081 phase tests — 100-day (2027-03-15) and 47-day (2029-03-15)
// using DER fixtures
// ---------------------------------------------------------------------------
//
// Oracle: CA/B Forum Ballot SC-081 effective dates (python3 calendar.timegm):
//   2027-03-16T00:00:00Z = 1_805_155_200  (one day past 100d threshold)
//   2029-03-16T00:00:00Z = 1_868_313_600  (one day past 47d threshold)
//
// Fixtures:
//   leaf-p256-110d-post-sc081-100d.der: notBefore=2027-03-16, 110 days (exceeds 100d cap)
//   leaf-p256-50d-post-sc081-100d.der:  notBefore=2027-03-16, 50 days  (within 100d cap)
//   leaf-p256-50d-post-sc081-47d.der:   notBefore=2029-03-16, 50 days  (exceeds 47d cap)
//   leaf-p256-45d-post-sc081-47d.der:   notBefore=2029-03-16, 45 days  (within 47d cap)
//
// All fixtures generated by pkix-path/tests/fixtures/policy-checks/gen-sc081-fixtures.py
// using pyca/cryptography; verified with openssl x509 -inform DER -noout -text.

/// 2027-03-16T00:00:00Z — one day after SC-081 100-day threshold
const T_AFTER_SC081_100D: u64 = 1_805_155_200;

/// 2029-03-16T00:00:00Z — one day after SC-081 47-day threshold
const T_AFTER_SC081_47D: u64 = 1_868_313_600;

#[test]
fn validity_max_error_110d_in_100d_phase() {
    // 110-day cert with notBefore in the 100-day phase: exceeds the 100-day cap.
    // Oracle: 110 days = 9_504_000 s > 100 days = 8_640_000 s → Error.
    let cert = load_cert!("leaf-p256-110d-post-sc081-100d.der");
    let lint = ValidityMaxLint;
    let result = lint.check_cert(&cert, SubjectKind::Leaf, T_AFTER_SC081_100D);
    assert!(
        matches!(result, LintResult::Error(_)),
        "110-day cert in 100-day phase must Error; got {result:?}"
    );
}

#[test]
fn validity_max_pass_50d_in_100d_phase() {
    // 50-day cert with notBefore in the 100-day phase: within the 100-day cap.
    // Oracle: 50 days = 4_320_000 s <= 100 days = 8_640_000 s → Pass.
    let cert = load_cert!("leaf-p256-50d-post-sc081-100d.der");
    let lint = ValidityMaxLint;
    let result = lint.check_cert(&cert, SubjectKind::Leaf, T_AFTER_SC081_100D);
    assert!(
        matches!(result, LintResult::Pass),
        "50-day cert in 100-day phase must Pass; got {result:?}"
    );
}

#[test]
fn validity_max_error_50d_in_47d_phase() {
    // 50-day cert with notBefore in the 47-day phase: exceeds the 47-day cap.
    // Oracle: 50 days = 4_320_000 s > 47 days = 4_060_800 s → Error.
    let cert = load_cert!("leaf-p256-50d-post-sc081-47d.der");
    let lint = ValidityMaxLint;
    let result = lint.check_cert(&cert, SubjectKind::Leaf, T_AFTER_SC081_47D);
    assert!(
        matches!(result, LintResult::Error(_)),
        "50-day cert in 47-day phase must Error; got {result:?}"
    );
}

#[test]
fn validity_max_pass_45d_in_47d_phase() {
    // 45-day cert with notBefore in the 47-day phase: within the 47-day cap.
    // Oracle: 45 days = 3_888_000 s <= 47 days = 4_060_800 s → Pass.
    let cert = load_cert!("leaf-p256-45d-post-sc081-47d.der");
    let lint = ValidityMaxLint;
    let result = lint.check_cert(&cert, SubjectKind::Leaf, T_AFTER_SC081_47D);
    assert!(
        matches!(result, LintResult::Pass),
        "45-day cert in 47-day phase must Pass; got {result:?}"
    );
}
