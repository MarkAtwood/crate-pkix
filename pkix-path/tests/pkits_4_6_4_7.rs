...
/// "
/// ```
/// (See pkix-path/tests/fixtures/gen-corrupt-bc.py if present, or the test comment in git.)
#[test]
fn basic_constraints_corrupt_der_returns_malformed_certificate() {
    let manifest = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let load = |name: &str| {
        let der = std::fs::read(manifest.join("tests/fixtures").join(name))
            .unwrap_or_else(|e| panic!("read {name}: {e}"));
        Certificate::from_der(&der).unwrap_or_else(|e| panic!("parse {name}: {e}"))
    };

    let root = load("corrupt-bc-root.der");
    let intermediate = load("corrupt-bc-int.der");
    let leaf = load("corrupt-bc-leaf.der");
