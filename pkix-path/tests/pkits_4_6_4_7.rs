...
/// must return `MalformedCertificate`, not `NotCA` and not `Ok`.
///
/// Fixtures committed to `tests/fixtures/` as `corrupt-bc-{root,int,leaf}.der`.
/// Generation script: `tests/fixtures/gen-corrupt-bc.py` (in git history).
#[test]
fn basic_constraints_corrupt_der_returns_malformed_certificate() {
    let load = |der: &[u8], name: &str| {
        Certificate::from_der(der).unwrap_or_else(|e| panic!("parse {name}: {e}"))
    };

    let root = load(
        include_bytes!("fixtures/corrupt-bc-root.der"),
        "corrupt-bc-root.der",
    );
    let intermediate = load(
        include_bytes!("fixtures/corrupt-bc-int.der"),
        "corrupt-bc-int.der",
    );
    let leaf = load(
        include_bytes!("fixtures/corrupt-bc-leaf.der"),
        "corrupt-bc-leaf.der",
    );
